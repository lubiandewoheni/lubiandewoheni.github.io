...
int main(int argc, char** argv) {
	...
	if(argc < 4) {
		printf("\n--------------------------------");
		printf("\n使用: ConsolePauser.exe <0|1> <shared_memory_id> <文件名> <参数>\n");
		printf("\n 1 表示 STDIN 由 Red Panda C++ 重定向; 0 表示不是\n");
	...
	if ( hJob == NULL ) {
		printf( "CreateJobObject failed: 错误 %d\n", GetLastError() );
		return 0;
	}
	...
	if ( bSuccess == FALSE ) {
		printf( "SetInformationJobObject failed: 错误 %d\n", GetLastError() );
		return 0;
	}
	...
	printf("\n--------------------------------");
	printf("\n执行时间：%.4g s\n程序返回：%u (0x%X)\n内存使用：%lld KB\n",seconds,returnvalue,returnvalue,peakMemory);
	if (pauseAfterExit)
